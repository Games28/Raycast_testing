...
#define FLOOR_3QRTR 'T'    //                 3/4
    ...
    bool OnUserCreate() override {
        ...
        pWallSprite = load_sprite_file("bluestone.png"); bSuccess &= (pWallSprite != nullptr);
        pFloorSprite = load_sprite_file("graystone.png"); bSuccess &= (pFloorSprite != nullptr);
        pCeilSprite = load_sprite_file("colorstone.png"); bSuccess &= (pCeilSprite != nullptr);
        pRoofSprite = load_sprite_file("colorstone.png"); bSuccess &= (pRoofSprite != nullptr);

        // load sprites for rendering objects
        pObjectSprite = load_sprite_file("r2idletest.png"); bSuccess &= (pObjectSprite != nullptr);
...
#define    ROOF_DRAWING 4
        ...
        for (auto& object : vListObjects) {
            ...
            // screen boundaries)
            bool bInFOV = fabs(fObjA) < fPlayerFoV_rad / 1.0f;
                ...
                for (float fx = 0.0f; fx < fObjWidth; fx++) {
                    ...
                    if (nObjColumn >= 0 && nObjColumn < ScreenWidth()) {
                        for (float fy = 0.0f; fy < fObjHeight; fy++) {
                            ...
                            if (pSample != olc::MAGENTA && fDepthBuffer[nObjColumn] >= fObjDist) {
